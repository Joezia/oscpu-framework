...
always @(posedge clk)begin
	if(!rst)begin
		...
		if(state_mem && rw_trans_done_i)begin
			mem_data_read_o		<= rw_data_read_i;
			situation_if_valid	<= if_valid_i ? 1 : 0;
		end
		if(state_if && rw_r_hs_i)			if_data_read_o		<= rw_data_read_i;
...
always@(posedge clk)begin
	if(!rst)begin
		...
		if		(rw_trans_done_i && state_mem && (!if_valid_i))	axi_stall_o <= 1'b0;
