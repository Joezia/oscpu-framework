...
module ysyx_040510_IDU(					//combinatorial logic
	...
	output			if_cmov,
	input			lb_rs1_value_i,
	input			lb_rs2_value_i,
	input			lb_rd_value_i,
	output [3:0]	lb_isa_type,
	output [4:0]	lb_rs1_o,
	output [4:0]	lb_rs2_o,
	output [4:0]	lb_rd_o,
	output			lb_wen_o,
	...
	wire oisa_seal = ~func3[2] & ~func3[1] & ~func3[0] & opcode[6] & ~opcode[5] & opcode[4] & ~opcode[3] & opcode[2] & opcode[1] & opcode[0];
	wire oisa_unseal = ~func3[2] & ~func3[1] & ~func3[0] & opcode[6] & opcode[5] & opcode[4] & ~opcode[3] & opcode[2] & opcode[1] & opcode[0];
	wire oisa_cmov = inst[31] & ~inst[30] & ~inst[29] & ~inst[28] & ~inst [27]& ~inst[26] & ~inst[25] & ~func3[2] & ~func3[1] & ~func3[0] & ~opcode[6] & opcode[5] & opcode[4] & ~opcode[3] & ~opcode[2] & opcode[1] & opcode[0];
	wire oisa_cmovw = inst[31] & ~inst[30] & ~inst[29] & ~inst[28] & ~inst [27]& ~inst[26] & ~inst[25] & ~func3[2] & ~func3[1] & ~func3[0] & ~opcode[6] & opcode[5] & opcode[4] & opcode[3] & ~opcode[2] & opcode[1] & opcode[0];
	...
	//**ALU**
	assign alu_op[0]=inst_aes64ds|inst_aes64dsm|inst_aes64es|inst_aes64im|inst_aes64esm|inst_aes64ks2|inst_aes64ks1i|inst_sub|inst_subw|inst_slti|inst_sltiu|inst_beq|inst_bne|inst_sll|inst_sllw|inst_remuw|inst_sltu|inst_or|inst_ori|inst_srli|inst_srliw|inst_divuw|inst_bltu|inst_slt|inst_sra|inst_sraw;
	assign alu_op[1]=inst_aes64ds|inst_aes64dsm|inst_aes64es|inst_aes64im|inst_aes64esm|inst_aes64ks2|inst_aes64ks1i|inst_csrrc|inst_csrrs|inst_csrrci|inst_csrrsi|inst_csrrw|inst_csrrwi|inst_srai|inst_sraiw|inst_xor|inst_xori|inst_ori|inst_or|inst_mulw|inst_mul|inst_remw|inst_divuw|inst_slti|inst_slt|inst_sra|inst_sraw;
	assign alu_op[2]=inst_andi|inst_sll|inst_sllw|inst_and|inst_xor|inst_xori|inst_ori|inst_or|inst_divw|inst_remw|inst_remuw|inst_slti|inst_slt|oisa_cmov;
	...
	assign alu_op[4]=inst_aes64ds|inst_aes64dsm|inst_aes64es|inst_aes64im|inst_aes64esm|inst_aes64ks2|inst_aes64ks1i|inst_csrrc|inst_csrrs|inst_csrrci|inst_csrrsi|inst_csrrw|inst_csrrwi|inst_srl|inst_srlw|inst_sltiu|inst_sltu|oisa_cmov;
	...
	assign alu_imm_reg2=inst_slli|inst_slliw|inst_srli|inst_srliw|inst_sd|inst_sw|inst_sh|inst_sb|inst_andi|inst_addi|inst_xori|inst_ori|inst_addiw|inst_auipc|inst_lui|inst_lb|inst_lbu|inst_lhu|inst_lwu|inst_lw|inst_lh|inst_ld|inst_slti|inst_sltiu|inst_srai|inst_sraiw|oisa_seal|oisa_unseal;
	...
	//OISA
	assign if_cmov = oisa_cmov;
	assign lb_rs1_o = rs1; 
	assign lb_rs2_o = rs2;
	assign lb_rd_o  = rd;

	assign lb_isa_type[0] = inst_addi|inst_xori|inst_ori|inst_addiw|inst_slti|inst_sltiu|inst_srai|inst_sraiw|inst_andi|inst_slli|inst_slliw|inst_srli|inst_srliw|oisa_seal|inst_beq|inst_bge|inst_bltu|inst_bne|inst_bgeu|inst_blt|inst_jal;
	assign lb_isa_type[1] = oisa_unseal|oisa_seal|inst_jal|inst_jalr; 
	assign lb_isa_type[2] = oisa_cmov|inst_beq|inst_bge|inst_bltu|inst_bne|inst_bgeu|inst_blt|inst_jal|inst_jalr; 
	assign lb_isa_type[3] = inst_xor|inst_subw|inst_addw|inst_remuw|inst_remw|inst_divw|inst_divuw|inst_mulw|inst_mul|inst_sub|inst_add|inst_sra|inst_sraw|inst_srl|inst_srlw|inst_and|inst_or|inst_sllw|inst_sll|inst_sltu|inst_slt;

	assign lb_wen_o = (lb_isa_type == `OISA_ARI_R)|(lb_isa_type == `OISA_ARI_I)|(lb_isa_type == `OISA_UNSEAL)|(lb_isa_type == `OISA_SEAL)|(lb_isa_type == `OISA_CMOV)|(lb_isa_type == `OISA_JALR)|(lb_isa_type == `OISA_JAL);

	assign lb_rd_value_o =	(lb_isa_type == `OISA_ARI_R)	& (lb_rs1_value_i | lb_rs2_value_i)					|
							(lb_isa_type == `OISA_ARI_I)	& (lb_rs1_value_i)									|
							(lb_isa_type == `OISA_UNSEAL)	& (1'b0)											|
							(lb_isa_type == `OISA_SEAL)		& (1'b1)											|
							(lb_isa_type == `OISA_CMOV)		& (lb_rs1_value_i | lb_rs2_value_i | lb_rd_value_i)	|
							(lb_isa_type == `OISA_JALR)		& (1'b0)											|
							(lb_isa_type == `OISA_JAL)		& (1'b0);
