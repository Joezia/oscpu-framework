	...
	if(!rst) begin
		...
		case(dcache_state)
			IDLE:begin
				...
				else if(mem_valid_i & (mem_req_i==`REQ_READ) & miss & is_dirty)begin
					dcache_state	<= MEM_WRITE_PRE;
					dcache_ram_addr <= {tag[index_i],index_i,3'b0}; 
				end
				else if(mem_valid_i & (mem_req_i==`REQ_READ) & miss & !is_dirty)begin
				...
				else if(mem_valid_i & (mem_req_i==`REQ_WRITE) & miss & is_dirty)begin
					dcache_state	<= MEM_WRITE_PRE;
					dcache_ram_addr <= {tag[index_i],index_i,3'b0};
				end
				else if(mem_valid_i & (mem_req_i==`REQ_WRITE) & miss & !is_dirty)begin
			...
			WRITE_HIT:begin
				if(arb_state_i == 2'b00)begin
					dcache_state	<= IDLE;
					tag	 [dcache_ram_addr[`INDEX_RANGE_D]]	<= dcache_ram_addr[`TAG_RANGE_D];
					valid[dcache_ram_addr[`INDEX_RANGE_D]]	<= 1'b1;
					dirty[dcache_ram_addr[`INDEX_RANGE_D]]	<= 1'b1;
				end
			end
			READ_HIT:begin
				if(arb_state_i == 2'b00)begin
			...
			CACHE_READ_FROM_MEM:begin		// note: Need to write dcache_ram !!!
				if(arb_mem_trans_done_i)begin
					tag	 [dcache_ram_addr[`INDEX_RANGE_D]]	<= dcache_ram_addr[`TAG_RANGE_D];
					valid[dcache_ram_addr[`INDEX_RANGE_D]]	<= 1'b1;
					dirty[dcache_ram_addr[`INDEX_RANGE_D]]	<= 0;
			...
			/* write to the memory - related output signal enable*/ 
				if(tmp_counter == 0) begin
					...
					dcache_data_o	<= dcache_ram_addr[3] ? dram_data_read_128[127:64] : dram_data_read_128[63:0];
			...
			CACHE_WRITE_BACK:begin			// note: Need to read dcache_ram !!!
				if(arb_mem_trans_done_i)begin
					dcache_state							<= IDLE;
					dirty[dcache_addr_o[`INDEX_RANGE_D]]	<= 0;
					valid[dcache_addr_o[`INDEX_RANGE_D]]	<= 0;
...
wire			dram_wen = !(	(dcache_state_cache_read_from_mem && (arb_mem_r_hs_i | arb_mem_trans_done_i)) |
								(dcache_state_write_hit)
							);
...
assign mem_data_read_o = mem_addr_i[3] ? dram_data_read_128[127:64] : dram_data_read_128[63:0];
