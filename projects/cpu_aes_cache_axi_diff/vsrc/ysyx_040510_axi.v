...
module ysyx_040510_axi# (
	...
	output                              rw_ready_o,
    input                               rw_req_i,
    output 	[RW_DATA_WIDTH:0]			data_read_o,
    input	[RW_DATA_WIDTH:0]			data_write_i,
    input  [AXI_DATA_WIDTH/8-1:0]       mask_write_i,
    input  [AXI_DATA_WIDTH:0]           rw_addr_i,
	input  [7:0]						rw_len_i,
    input  [1:0]                        rw_size_i,
	input  [1:0]						rw_arb_state_i,
    output [1:0]                        rw_resp_o,
	output								rw_r_hs_o,
	output								rw_trans_done_o,
    ...
    wire w_valid    = rw_valid_i & (rw_req_i == `REQ_WRITE);
    wire r_valid    = rw_valid_i & (rw_req_i == `REQ_READ);
    ...
    wire trans_done = rw_req_i == `REQ_WRITE ? b_hs : r_done;

	assign rw_trans_done_o = trans_done;
	assign rw_r_hs_o	= r_hs;
    ...
    always @(posedge clock) begin
        if (!rst) begin
            ...
            if (r_valid) begin			//r_valid = rw_valid_i & (rw_req_i == `REQ_READ)
                case (r_state)
                    R_STATE_IDLE:               r_state <= R_STATE_ADDR;
                    R_STATE_ADDR: if (ar_hs)    r_state <= R_STATE_READ;		//[ar_hs] ar -> r
                    R_STATE_READ: if (r_done)   r_state <= R_STATE_IDLE;		//r_hs && last
                    default:;
                endcase
            end
        end
	...
	parameter TRANS_LEN     = RW_DATA_WIDTH / AXI_DATA_WIDTH;	//64/64 = 1
...
//    wire [7:0] axi_len      = aligned ? TRANS_LEN - 1 : {{7{1'b0}}, overstep};
    ...
    reg [7:0] len;
	wire len_reset      = !rst | (rw_req_i == `REQ_WRITE & w_state_idle) | (rw_req_i == `REQ_READ & r_state_idle);
    ...
    reg [1:0] rw_resp;
    wire rw_resp_nxt = (rw_req_i==`REQ_WRITE) ? axi_b_resp_i : axi_r_resp_i;
    ...
    assign axi_aw_addr_o    = axi_addr;			//!!!!!!!!!!!!!!rw_addr_i;
    ...
    assign axi_w_strb_o     = mask_write_i;			//!!!!!!!!!!!!!!!!!rw_size_i;
    assign axi_w_last_o     = 1'b1;					//!!!!!!!!!!!!!!!! 1'b0;
    ...
    assign axi_ar_len_o     = rw_arb_state_i==2'b01 ? axi_len : rw_len_i;//没有添加dcache
